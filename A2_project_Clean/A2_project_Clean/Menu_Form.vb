Public Class Menu_Form 'this form is the menu option that allows you to pause and configure the program
    ...
    Private Sub Show_Creature_Click(sender As Object, e As EventArgs) Handles Show_Creature.Click 'when the show best creature button is clicked it activates this function
        ...
        Creature = Form1.CreatureDatabase.QueryDatabase("SELECT * FROM Creatures WHERE Distance = (SELECT MAX(Distance) FROM Creatures)", False, 12) 'retrieves the info from the database to be used
        Legs = Form1.CreatureDatabase.QueryDatabase("SELECT * FROM Legs WHERE Creature_ID = '" & Creature(0) & "'", False, 12)
